...
function Eliminar({ tipoEliminar }) {
  ...
  function handleSubmit(event) {
    ...
    $.ajax({
      url: 'http://localhost:8080/api/eliminarProducto', // URL para actualizar el producto
      method: 'DELETE',
    ...
    <div className='consulta-actualiza-principal'>
      ...
      <div className='contenedor-div'>
        ...
        <form className='derecho-actualiza' id="registerForm" onSubmit={handleSubmit}>
          ...
          <button className='btn-actualizar' type="submit" disabled={!isClaveSeleccionada}>Eliminar</button>
        </form>
      </div>
      <div className="estatus-actualizar">
        {eliminacionExitosa && <p className="registro-exitoso">Registro eliminado exitosamente.</p>}
      </div>
    </div>
  );
}
